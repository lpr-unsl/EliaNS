...
function Iniciar(){
	temp=$2	
	size=${#temp}
	version_lpr=`cat /root/EliaNS/version.txt`
	case "$temp" in 
		servidor)
			imagen="--privileged servidor:$version_lpr";;
		cliente)
			imagen="--env="DISPLAY" --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" cliente:$version_lpr";;
		cliente-cli)
			imagen="cliente-cli:$version_lpr";;
		router)
			imagen="router:$version_lpr";;
		*) echo "error en levantar los contenedores de docker con la version $version_lpr"; exit;;
	esac
